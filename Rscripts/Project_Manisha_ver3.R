...
## load the dataset
creditRisk <- read.csv("credit_risk_dataset.csv")
...
plot2 <- ggplot(creditRisk, aes(x = loan_intent, fill = as.factor(loan_status))) +
  geom_bar(position = "fill") + 
  labs(x = "Purpose of loan", y = "Proportion of loan approval", 
...
plot3 <- ggplot(creditRisk, aes(x = loan_grade, fill = as.factor(loan_status))) +
  geom_bar(position = "fill") + 
  labs(x = "Loan Grade", y = "Proportion of loan approval", 
...
plot4 <- ggplot(creditRisk, aes(x = cb_person_default_on_file, fill = as.factor(loan_status))) +
  geom_bar(position = "fill") + 
  labs(x = "Default History", y = "Proportion of loan approval", 
...
person_income <- log(creditRisk$person_income)
...
set.seed(123)
train_index <- createDataPartition(creditRisk$loan_status, p = 0.8, list = FALSE)
train_data <- creditRisk[train_index, ]
test_data <- creditRisk[-train_index, ]

colnames(train_data)
formula <- loan_status ~ person_age + person_income + person_home_ownership + 
...
test_data$predicted_class <- ifelse(test_data$predicted_prob > 0.6, 1, 0)


# Confusion matrix
conf_matrix <- confusionMatrix(factor(test_data$predicted_class), 
                               factor(test_data$loan_status), 
                               positive = "1")
...
library(ROCR)
pred <- prediction(test_data$predicted_prob, test_data$loan_status)

# Compute performance measures (AUC and ROC)
...
print(paste("AUC:", auc_value@y.values[[1]]))
